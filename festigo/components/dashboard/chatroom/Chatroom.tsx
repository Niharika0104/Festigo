...
export function Chatroom() {
  ...
  return (
    <div>
      <div className="h-screen flex justify-start  items-start gap-8 w-full">
        {/* chat-lists */}
        <div className="w-[350px] shrink-0 h-full bg-white px-2 flex flex-col gap-5 py-8">
          {/* Search bar */}
          ...
          {/* chat-lists */}
          <div className="flex flex-col p-4 w-full gap-2 justify-start items-start">
            {data.map((chat) => {
              return (
                <div
                  key={chat.id}
                  className={`w-full flex justify-between items-start rounded-xl text-[#080707] bg-white hover:bg-[#D9D9D9] p-3`}
                >
                  {/* profile | name | chat  */}
