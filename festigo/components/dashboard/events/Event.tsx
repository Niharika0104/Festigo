...
export function Event() {
  ...
  const onChatClick = async (email: string) => {
    const result = await axios.post(
      `${process.env.NEXT_PUBLIC_BASE_URL}/api/auth/emailExists`,
      {
        email: email,
      }
    );
            ...
            {vendors?.map((item: any, idx) => {
              return (
                <tr key={idx}>
                  ...
                  <td className="px-4 py-2">
                    {item.dateOfOrder?.split("T")[0]}
                  </td>
                  <td className="px-4 py-2">
                    <button
                      className="rounded-full px-4 py-2 w-[120px] bg-green-300 flex items-center justify-center gap-2"
                      onClick={() => {
                        onChatClick(item?.email);
                      }}
                    >
                      <Image
                        src={chatImage}
                  ...
                  <td className="px-4 py-2">
                    <button
                      className="rounded-full px-4 py-2 w-[120px] bg-yellow-300 flex items-center justify-center gap-2"
                      onClick={() => {
                        setModalOpen(true);
                        OnEditClick(item);
                      }}
